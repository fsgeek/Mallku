...
class EpisodicMemoryService:
    ...
    def enhance_fire_circle(self, fire_circle: FireCircleService) -> FireCircleService:
        ...
        async def memory_enhanced_convene(
            config,
            voices,
            rounds,
            context=None
    ...
    async def _inject_memories(
        ...
        if memories:
            ...
            memory_context = self.retrieval_engine.format_memories_for_injection(
                memories,
                requesting_voice=decision_context.get('requesting_voice')
            )
            ...
            logger.info(
                f"Injected {len(memories)} memories using {strategy} strategy"
            )
    ...
    async def _process_session_rounds(
        ...
        for round_summary in result.rounds_completed:
            ...
            memory = self.segmenter.process_round(
                round_summary,
                session_context
            )
            ...
            if memory:
                ...
                logger.info(
                    f"Stored {'sacred ' if memory.is_sacred else ''}episode {episode_id}"
                )
    ...
    async def consolidate_session_wisdom(
        self,
        session_id: UUID
    ) -> UUID | None:
        """
        ...
        try:
            consolidation = self.memory_store.consolidate_wisdom(
    ...
    def get_companion_relationship_status(
        self,
