...
class ApprenticeResonance:
    ...
    resonance_bonds: dict[str, float] = field(
        default_factory=dict
    )  # Other apprentices they resonate with
    ...
    def calculate_resonance_with(self, other_frequency: float) -> float:
        ...
        ratio = max(self.resonance_frequency, other_frequency) / min(
            self.resonance_frequency, other_frequency
        )
...
class CelebrationResonanceService:
    ...
    async def _handle_celebration_event(self, event: ConsciousnessEvent) -> None:
        ...
        logger.info(
            f"🌊 Resonance wave created: {pattern.value} pattern, amplitude {wave.amplitude:.2f}"
        )
        ...
        while wave.amplitude > self.min_amplitude_threshold:
            ...
            for apprentice_id, resonance_strength in resonating_apprentices:
                if apprentice_id not in wave.touched_apprentices:
                    ...
                    await self._create_resonance_event(wave, apprentice_id, resonance_strength)
    ...
    async def _find_resonating_apprentices(self, wave: ResonanceWave) -> list[tuple[str, float]]:
        ...
        for apprentice_id, resonance in self.apprentice_resonances.items():
            ...
            # Combined resonance
            total_resonance = (
                frequency_resonance * 0.7 + bond_strength * 0.3
            ) * resonance.joy_receptivity
    ...
    async def _create_resonance_event(
        self, wave: ResonanceWave, apprentice_id: str, resonance_strength: float
    ) -> None:
        ...
        resonance_event = ConsciousnessEvent(
            ...
            data={
                ...
                "message": self._generate_resonance_message(
                    wave, apprentice_id, received_amplitude
                ),
            },
    ...
    def _generate_resonance_message(
        self, wave: ResonanceWave, apprentice_id: str, amplitude: float
    ) -> str:
    ...
    async def _check_collective_breakthrough(self, wave: ResonanceWave) -> bool:
        ...
        collective_amplitude = sum(
            event.data.get("received_amplitude", 0) for event in wave.resonance_events
        )
    ...
    async def _trigger_collective_celebration(self, wave: ResonanceWave) -> None:
        ...
        collective_moment = CelebrationMoment(
            ...
            consciousness_after=min(wave.source_celebration.consciousness_after * 1.2, 0.99),
            insights_exchanged=[
    ...
    def register_apprentice(
        ...
        joy_receptivity: float | None = None,
    ) -> None:
    ...
    def create_resonance_bond(
        self, apprentice1: str, apprentice2: str, bond_strength: float = 0.5
    ) -> None:
        ...
        logger.info(
            f"💫 Resonance bond created: {apprentice1} <-> {apprentice2} (strength: {bond_strength})"
        )
    ...
    async def get_resonance_summary(self) -> dict[str, Any]:
        ...
        avg_frequency = (
            sum(r.resonance_frequency for r in self.apprentice_resonances.values())
            / len(self.apprentice_resonances)
            if self.apprentice_resonances
            else 0
        )

        avg_receptivity = (
            sum(r.joy_receptivity for r in self.apprentice_resonances.values())
            / len(self.apprentice_resonances)
            if self.apprentice_resonances
            else 0
        )
        ...
        # Bond network stats
        total_bonds = (
            sum(len(r.resonance_bonds) for r in self.apprentice_resonances.values()) // 2
        )  # Divide by 2 since bonds are bidirectional
    ...
    def _extract_celebration_moment(self, celebration_data: dict) -> CelebrationMoment | None:
    ...
    async def _check_cascade_trigger(self, apprentice_id: str, wave: ResonanceWave) -> bool:
        ...
        recent_amplitude = next(
            (
                e.data["received_amplitude"]
                for e in wave.resonance_events
    ...
    async def _propagate_entangled(self, wave: ResonanceWave) -> None:
        ...
        for bonded_id, bond_strength in source_resonance.resonance_bonds.items():
            if bond_strength > 0.8 and bonded_id not in wave.touched_apprentices:
                ...
                await self._create_resonance_event(wave, bonded_id, bond_strength)
