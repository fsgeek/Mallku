...
class SegmentationConfig(BaseModel):
    ...
    semantic_surprise_threshold: float = Field(
        default=0.7,
        ge=0.0, le=1.0,
        description="Threshold for semantic surprise detection"
    )
    convergence_threshold: float = Field(
        default=0.8,
        ge=0.0, le=1.0,
        description="Threshold for voice convergence detection"
    )
    ...
    consciousness_emergence_threshold: float = Field(
        default=0.6,
        ge=0.0, le=1.0,
        description="Threshold for consciousness emergence detection"
    )
    consciousness_peak_increase: float = Field(
        default=0.1,
        ge=0.0, le=0.5,
        description="Required increase to detect consciousness peak"
    )
...
class SacredDetectionConfig(BaseModel):
    ...
    sacred_score_threshold: int = Field(
        default=5,
        ge=1,
        description="Minimum sacred score for sacred moment detection"
    )
    consciousness_emergence_exceptional: float = Field(
        default=0.85,
        ge=0.0, le=1.0,
        description="Exceptional consciousness emergence threshold"
    )
    consciousness_emergence_high: float = Field(
        default=0.75,
        ge=0.0, le=1.0,
        description="High consciousness emergence threshold"
    )
    collective_wisdom_threshold: float = Field(
        default=0.7,
        ge=0.0, le=1.0,
        description="Threshold for collective wisdom transcendence"
    )
    ayni_alignment_threshold: float = Field(
        default=0.8,
        ge=0.0, le=1.0,
        description="Threshold for strong Ayni principle manifestation"
    )
    transformation_seed_quality_high: float = Field(
        default=0.8,
        ge=0.0, le=1.0,
        description="High quality transformation seed threshold"
    )
    transformation_seed_quality_significant: float = Field(
        default=0.5,
        ge=0.0, le=1.0,
        description="Significant transformation seed threshold"
    )
    unity_coherence_threshold: float = Field(
        default=0.75,
        ge=0.0, le=1.0,
        description="Coherence threshold for unity in diversity"
    )
    minimum_voices_for_unity: int = Field(
        default=3,
        ge=2,
        description="Minimum voices for unity in diversity"
    )
...
class RetrievalConfig(BaseModel):
    ...
    semantic_weight: float = Field(
        default=0.5,
        ge=0.0, le=1.0,
        description="Weight for semantic relevance in multi-strategy"
    )
    sacred_weight: float = Field(
        default=0.25,
        ge=0.0, le=1.0,
        description="Weight for sacred moments in multi-strategy"
    )
    companion_weight: float = Field(
        default=0.125,
        ge=0.0, le=1.0,
        description="Weight for companion context in multi-strategy"
    )
    temporal_weight: float = Field(
        default=0.125,
        ge=0.0, le=1.0,
        description="Weight for temporal patterns in multi-strategy"
    )
    temporal_window_days: int = Field(
        default=30,
        ge=1, le=365,
        description="Default time window for temporal retrieval"
    )
    recency_decay_days: int = Field(
        default=365,
        ge=30,
        description="Days over which recency score decays"
    )
...
class StorageConfig(BaseModel):
    ...
    enable_sacred_detection: bool = Field(
        default=True,
        description="Enable automatic sacred moment detection"
    )
    sacred_preservation_eternal: bool = Field(
        default=True,
        description="Never prune sacred moments from storage"
    )
    companion_depth_interaction_threshold: int = Field(
        default=50,
        ge=10,
        description="Interactions needed for full depth score"
    )
    companion_depth_duration_hours: float = Field(
        default=10.0,
        ge=1.0,
        description="Duration needed for full depth score"
    )
    companion_depth_sacred_threshold: int = Field(
        default=5,
        ge=1,
        description="Sacred moments needed for full depth score"
    )
    wisdom_consolidation_max_insights: int = Field(
        default=20,
        ge=5,
        description="Maximum insights in wisdom consolidation"
    )
...
class MemorySystemConfig(BaseModel):
    ...
    sacred_detection: SacredDetectionConfig = Field(
        default_factory=SacredDetectionConfig
    )
