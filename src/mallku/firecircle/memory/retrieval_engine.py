...
class CompanionAwareStrategy(RetrievalStrategy):
    ...
    def retrieve(
        ...
        companion_memories = store.retrieve_companion_memories(
            human_id,
            limit=limit // 2
        )
...
class MemoryRetrievalEngine:
    ...
    def retrieve_for_decision(
        ...
        logger.info(
            f"Retrieved {len(memories)} memories using {strategy_name} strategy"
        )
    ...
    def retrieve_multi_strategy(
        ...
        for strategy_name, allocation in allocations.items():
            ...
            memories = self.retrieve_for_decision(
                decision_context,
                strategy_name,
                allocation
            )
        ...
        all_memories.sort(
            key=lambda m: m.consciousness_indicators.overall_emergence_score,
            reverse=True
        )
    ...
    def format_memories_for_injection(
        ...
        return {
            ...
            'consciousness_peak': max(
                m.consciousness_indicators.overall_emergence_score
                for m in memories
            )
        }
    ...
    def suggest_retrieval_strategy(
        ...
        wisdom_indicators = [
            'architecture', 'consciousness', 'transformation',
            'fundamental', 'sacred', 'wisdom', 'civilizational'
        ]
