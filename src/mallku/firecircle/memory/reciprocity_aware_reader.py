...
class ReciprocityAwareMemoryReader(SharedMemoryReader):
    ...
    def __init__(
        ...
        self.current_exchange: MemoryExchange | None = None
        self.exchange_history: list[MemoryExchange] = []

    def search_with_awareness(
        self, keywords: set[str], limit: int = 10, need_context: dict[str, Any] | None = None
    ...
    def contribute_insights(self, insights: list[str], consciousness_score: float = 0.0) -> None:
        """
        ...
        if not self.current_exchange:
            logger.warning(
                f"Apprentice {self.apprentice_id} contributing insights without active exchange"
            )
            return
    ...
    def get_reciprocity_summary(self) -> dict[str, Any]:
        ...
        completed_exchanges = sum(1 for ex in self.exchange_history if ex.reciprocity_complete)

        total_memories_accessed = sum(len(ex.memories_accessed) for ex in self.exchange_history)
        ...
        avg_consciousness = (
            sum(ex.consciousness_score for ex in self.exchange_history) / total_exchanges
    ...
    def _record_memory_access(
        self, keywords: set[str], results: list[tuple[str, float]], need_context: dict[str, Any]
    ) -> None:
        ...
        try:
            ...
            InteractionRecord(
                ...
                needs_expressed=[self._map_need_category(need_context.get("purpose", ""))],
                needs_fulfilled=[NeedCategory.GROWTH if results else NeedCategory.MEANING],
                participant_context={
            ...
            logger.debug(
                f"Recorded memory access: {self.apprentice_id} accessed {len(results)} memories"
            )

        except Exception as e:
            logger.error(f"Failed to record memory access: {e}")

    def _record_insight_contribution(self, insights: list[str], consciousness_score: float) -> None:
    ...
    def _track_consciousness_evolution(self) -> list[float]:
        ...
        return [ex.consciousness_score for ex in self.exchange_history if ex.reciprocity_complete]
