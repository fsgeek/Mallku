...
from .heartbeat_service import HeartbeatConfig
...
class MemoryAwareHeartbeatService(EnhancedHeartbeatService):
    ...
    async def _monitor_memory_health(self) -> None:
        ...
        while self.is_beating:
            try:
                ...
                should_trigger, reason, template = (
                    MemoryCeremonyIntegration.should_trigger_ceremony(
                        memory_state, self.last_ceremony_timestamps, datetime.now(UTC).timestamp()
                    )
                )
                ...
                if time_based_template and not self.ceremony_in_progress:
                    ...
                    if time_since_last > 21600:
                        await self.trigger_memory_ceremony(
                            time_based_template, f"rhythmic_{ceremony_key}_time"
                        )
    ...
    async def trigger_memory_ceremony(self, template, reason: str) -> None:
        ...
        try:
            ...
            emergence_quality = self._calculate_ceremony_emergence(
                before_consciousness, after_consciousness, ceremony_result
            )
    ...
    async def _conduct_memory_ceremony(self, template, reason: str) -> dict[str, Any]:
        ...
        context = MemoryCeremonyIntegration.prepare_ceremony_context(
            template, memory_state, specific_patterns=memory_state.get("candidate_patterns", [])
        )
        ...
        if self.fire_circle_service:
            ...
            result = await self.fire_circle_service.facilitate_review(
                context_data=context, round_configs=template.rounds
            )
    ...
    def _calculate_ceremony_emergence(
        self, before: float, after: float, ceremony_result: dict[str, Any]
    ) -> float:
    ...
    async def validate_ceremony_effectiveness(self, ceremony_results: CeremonyResults) -> dict:
        ...
        wisdom = await facilitate_mallku_decision(
            ...
            context={
                ...
                "consciousness_change": ceremony_results.consciousness_after
                - ceremony_results.consciousness_before,
        ...
        return {
            "validation_score": wisdom.consciousness_score
            if hasattr(wisdom, "consciousness_score")
            else 0.8,
...
def create_memory_aware_heartbeat(
    ...
    return MemoryAwareHeartbeatService(
        config=config, event_bus=event_bus, memory_monitor=memory_monitor
    )
