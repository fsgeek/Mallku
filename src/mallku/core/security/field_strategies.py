...
class FieldSecurityConfig(BaseModel):
    ...
    index_strategy: FieldIndexStrategy = Field(
        default=FieldIndexStrategy.NONE, description="How the field is made queryable"
    )

    search_capabilities: list[SearchCapability] = Field(
        default_factory=list, description="Query types this field must support"
    )

    security_notes: str | None = Field(
        default=None, description="Documentation of security/utility trade-off"
    )

    # Strategy-specific configuration
    bucket_boundaries: list[float] | None = Field(
        default=None, description="For BUCKETED strategy: bucket boundaries"
    )

    temporal_precision: str | None = Field(
        default=None, description="For TEMPORAL_OFFSET: precision level (minute, hour, day)"
    )

    derivation_function: str | None = Field(
        default=None, description="For DERIVED: name of derivation function"
    )
    ...
    def validate_configuration(self) -> list[str]:
        ...
        if SearchCapability.RANGE in self.search_capabilities:
            ...
            if self.index_strategy not in valid_strategies:
                warnings.append(f"Strategy {self.index_strategy} may not support RANGE queries")

        if (
            SearchCapability.FULLTEXT in self.search_capabilities
            and self.obfuscation_level == FieldObfuscationLevel.ENCRYPTED
        ):
            warnings.append("FULLTEXT search on ENCRYPTED fields requires special handling")
