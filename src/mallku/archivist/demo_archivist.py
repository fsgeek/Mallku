...
async def create_demo_memory_anchors() -> list[MemoryAnchor]:
    ...
    morning_anchor = MemoryAnchor(
        ...
        cursor_state={
            "filesystem": {"path": "/projects/vision_doc.md", "event": "created"}
        },
        metadata={
    ...
    followup_anchor = MemoryAnchor(
        ...
        cursor_state={
            "filesystem": {"path": "/projects/vision_doc.md", "event": "modified"}
        },
    ...
    meeting_anchor = MemoryAnchor(
        ...
        cursor_state={
            "calendar": {"event": "Team Standup", "duration": 30}
        },
        metadata={
    ...
    reflection_anchor = MemoryAnchor(
        ...
        cursor_state={
            "filesystem": {"path": "/journal/daily_reflection.md", "event": "created"}
        },
        metadata={
...
async def demonstrate_queries(archivist: ArchivistService):
    ...
    for i, query_info in enumerate(queries, 1):
        ...
        # Display response
        formatted = await archivist.wisdom_synthesizer.format_for_display(
            response, "terminal"
        )
        print(formatted)
    ...
    for session in patterns.get("work_sessions", [])[:3]:
        print(f"   {session['start'].strftime('%H:%M')} - {session['end'].strftime('%H:%M')}: "
