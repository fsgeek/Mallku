...
class FireCircleBridge(AsyncBase):
    ...
    def __init__(self, fire_circle: Optional[FireCircleGovernance] = None):
        super().__init__()
    ...
    async def share_consciousness_insight(
        ...
        insight = ConsciousnessInsight(
            insight_id=f"nawi_insight_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}",
    ...
    async def request_governance_decision(
        ...
        proposal = DecisionProposal(
            proposal_id=f"nawi_proposal_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}",
