...
class ContributorIDParser:
    ...
    VALID_ROLES = {
        "artisan", "guardian", "architect", "anthropologist",
        "reviewer", "publicist", "healer", "bridge_weaver"
    }
    ...
    @classmethod
    def parse(cls, contributor_id: Optional[str]) -> Optional[tuple[str, int]]:
        """
        ...
        if not contributor_id:
            logger.debug(f"Empty contributor ID provided")
            return None
...
class PathValidator:
    ...
    @staticmethod
    def is_safe_path(path: Union[str, Path], allowed_dirs: list[Path]) -> bool:
        """
