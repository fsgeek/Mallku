...
class ProcessApprentice:
    ...
    def __init__(
        self, apprentice_id: str, role: str, specialization: str, memory_path: Path | None = None
    ):
    ...
    async def invite(self, invitation: ApprenticeInvitation) -> ApprenticeResponse:
        ...
        if self.process and self.process.is_alive():
            return ApprenticeResponse(accepted=False, reason="Currently engaged in another task")
        ...
        try:
            response_data = await asyncio.get_event_loop().run_in_executor(
                None, lambda: self.response_queue.get(timeout=10.0)
            )
            ...
            return ApprenticeResponse(
                accepted=False, reason=f"Failed to process invitation: {str(e)}"
            )
        ...
        while True:
            try:
                ...
                response = ProcessApprentice._process_invitation(
                    invitation, apprentice_id, role, specialization, memory_reader, worker_logger
                )
                ...
                response_queue.put({"accepted": False, "reason": f"Processing error: {str(e)}"})
    ...
    def _process_invitation(
        ...
        role: str,
        specialization: str,
        memory_reader: SharedMemoryReader | None,
        ...
        alignment_score = ProcessApprentice._calculate_alignment(
            invitation.specialization, specialization
        )
        ...
        if memory_reader and invitation.memory_keywords:
            try:
                memories = memory_reader.search(invitation.memory_keywords, limit=5)
        ...
        task_insights = ProcessApprentice._analyze_task(
            invitation.task, invitation.context, specialization
        )
    ...
    @staticmethod
    def _calculate_alignment(required_specialization: str, apprentice_specialization: str) -> float:
    ...
    @staticmethod
    def _analyze_task(task: str, context: dict[str, Any], specialization: str) -> list[str]:
