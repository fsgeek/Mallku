...
from .base_provider import ActivityProvider, ActivityEvent, ActivityType
...
class ConsciousnessFileHandler(FileSystemEventHandler):
    ...
    async def get_next_event(self) -> Optional[tuple]:
...
class FileSystemActivityProvider(ActivityProvider):
    ...
    def __init__(self, 
                 ...
                 event_bus=None,
                 privacy_extensions: Optional[Set[str]] = None):
    ...
    async def _create_activity_event(self,
                                   file_path: str,
                                   activity_type: ActivityType) -> Optional[ActivityEvent]:
        """
    ...
    def respects_privacy(self, path: str) -> bool:
        ...
        for parent in path_obj.parents:
            if parent.name.lower() in private_dirs:
                return False
