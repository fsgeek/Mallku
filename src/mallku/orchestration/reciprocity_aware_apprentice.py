...
class ReciprocityAwareApprentice(ProcessApprentice):
    ...
    async def invite_with_reciprocity(
        self,
    ...
    def _has_excessive_debt(self) -> bool:
        ...
        total_accessed = sum(
            len(ex.memories_accessed) for ex in self.pending_reciprocity
        )
    ...
    async def _ensure_reciprocity(
        ...
        if response.insights and response.confidence > self.reciprocity_threshold:
            ...
            self.pending_reciprocity = [
                ex for ex in self.pending_reciprocity
                if not ex.reciprocity_complete
            ]
    ...
    def get_reciprocity_status(self) -> dict[str, Any]:
        ...
        completed = sum(
            1 for ex in self.exchange_history if ex.reciprocity_complete
        )
...
class ReciprocityTrackingCoordinator:
    ...
    def _generate_recommendations(self) -> list[str]:
        ...
        if not recommendations:
            recommendations.append(
                "System reciprocity healthy - continue current patterns"
            )
