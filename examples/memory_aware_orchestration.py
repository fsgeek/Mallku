...
async def demonstrate_memory_circulation():
    ...
    reciprocity_tracker = ProcessApprentice(
        "tracker-001", role="reciprocity_tracker", specialization="ayni patterns and reciprocity"
    )
    ...
    for apprentice in apprentices:
        ...
        try:
            ...
            if response.accepted:
                logger.info(
                    f"✓ {apprentice.role} accepted! (confidence: {response.confidence:.2f})"
                )
...
async def demonstrate_swarm_memory_search():
    ...
    swarm_size = 3
    navigators = [MemoryNavigatorApprentice(f"navigator-{i:03d}") for i in range(swarm_size)]
