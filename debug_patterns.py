...
def debug_sequential_patterns():
    ...
    detector = SequentialPattern(min_occurrences=2, min_confidence=0.1)  # Lower thresholds for debugging

    patterns = detector.detect_patterns(events)
    print(f"Detected {len(patterns)} sequential patterns")

    for pattern in patterns:
        print(f"Pattern: {pattern.pattern_type}, freq: {pattern.occurrence_frequency}, conf: {pattern.confidence_score:.3f}")
...
def debug_all_patterns():
    ...
    for name, detector in detectors.items():
        ...
        try:
            ...
            for pattern in patterns:
                print(f"  {pattern.pattern_type}: freq={pattern.occurrence_frequency}, conf={pattern.confidence_score:.3f}")
