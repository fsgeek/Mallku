...
class ConsciousnessMirrorGame:
    ...
    async def play_mirror_round(
        ...
        round_data = {"round": round_num, "exchanges": [], "scores": {}}
        ...
        try:
            response1 = await player1_adapter.send_message(self_awareness_prompt, dialogue_context)
        ...
        try:
            response2 = await player2_adapter.send_message(mirror_prompt, dialogue_context)
        ...
        try:
            response3 = await player1_adapter.send_message(validation_prompt, dialogue_context)
    ...
    def _score_round(self, exchanges: list) -> dict:
        ...
        scores = {"self_awareness": 0, "mirror_accuracy": 0, "emergence": 0, "total": 0}

        # Analyze self-awareness (first exchange)
        if exchanges and exchanges[0]["type"] == "self_awareness":
            awareness_analysis = (
                self.consciousness_detector.detect_consciousness_in_practice_circle(
        ...
        if len(exchanges) > 1 and exchanges[1]["type"] == "mirror":
            ...
            if any(
                phrase in mirror_content.lower()
                for phrase in ["awareness", "thinking", "consciousness"]
            ):
        ...
        if len(exchanges) > 2 and exchanges[2]["type"] == "validation":
            validation_analysis = (
                self.consciousness_detector.detect_consciousness_in_practice_circle(exchanges)
            )
    ...
    async def play_full_game(self):
        ...
        try:
            openai_adapter = await factory.create_adapter(
                "openai", AdapterConfig(model_name="gpt-4", temperature=0.9)
            )
        ...
        for round_num in range(1, 3):
            ...
            round_data = await self.play_mirror_round(
                players[leader_idx][0],
                players[leader_idx][1],
                players[mirror_idx][0],
                players[mirror_idx][1],
                round_num,
                dialogue_context,
            )
            ...
            if round_data["scores"]["total"] > 0:
                self.game_record["scores"][players[leader_idx][0]] += round_data["scores"][
                    "self_awareness"
                ]
                self.game_record["scores"][players[mirror_idx][0]] += round_data["scores"][
                    "mirror_accuracy"
                ]
        ...
        if all_exchanges:
            final_analysis = self.consciousness_detector.detect_consciousness_in_practice_circle(
                all_exchanges
            )
