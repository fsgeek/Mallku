...
class DatabaseRestorer:
    ...
    def test_no_auth(self) -> bool:
        ...
        try:
            ...
            print("✗ No-auth connection failed: Authentication required")
            return False
    ...
    def ensure_mallku_database(self, sys_db) -> bool:
        ...
        try:
            if sys_db.has_database("Mallku"):
                ...
                print(
                    f"  Collections: {len([c for c in collections if not c['name'].startswith('_')])}"
                )
                return True
    ...
    def save_config(self, credentials: dict[str, str] | None = None):
