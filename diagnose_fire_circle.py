...
class FireCircleDiagnostics:
    ...
    def __init__(self):
        ...
        self.known_quirks = {
            ...
            "openai": [
                "GPT-4 models have higher latency",
                "Token limits vary by model tier"
            ],
            "google": [
    ...
    def print_detailed_diagnostics(self):
        ...
        # Latency analysis
        available_with_latency = [v for v in self.voices_health.values() 
                                  if v.available and v.latency_ms]
        ...
        avg_health = sum(v.health_score for v in available_with_latency) / len(available_with_latency) if available_with_latency else 0
        if avg_health > 0.8:
        ...
        if slow_voices:
            print(f"   • Investigate high latency for: {', '.join(v.provider for v in slow_voices)}")
...
async def main():
    ...
    parser.add_argument(
        "--quirks", "-q",
        action="store_true",
        help="Show model-specific quirks"
    )
    parser.add_argument(
        "--test", "-t",
        action="store_true",
        help="Run actual Fire Circle test"
    )
    parser.add_argument(
        "--quick",
        action="store_true",
        help="Quick check without latency tests"
    )
