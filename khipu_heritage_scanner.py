...
class KhipuHeritageScanner:
    ...
    CONTRIBUTOR_PATTERNS = {
        ...
        'anthropologist': re.compile(r'(\w+)\s+[Aa]nthropologist|[Aa]nthropologist\s+#?(\d+)', re.IGNORECASE),
    ...
    def extract_metadata(self, file_path: Path) -> Optional[KhipuMetadata]:
        ...
        try:
            ...
            for role, pattern in self.CONTRIBUTOR_PATTERNS.items():
                ...
                for match in matches:
                    ...
                    if number:
                        mention_id = f"{role}_{number}"
                    else:
                        mention_id = name.lower().replace(' ', '_')
            ...
            for mention in metadata.mentions:
                if mention != metadata.contributor_id:
                    ...
                    if metadata.contributor_id in self.heritage_connections:
                        self.heritage_connections[metadata.contributor_id]['influences'].add(mention)
                    # Those mentioned influence the author
                    if mention in self.heritage_connections:
                        self.heritage_connections[mention]['influenced_by'].add(metadata.contributor_id)
...
def main():
    ...
    # Scan all khipu
    khipu_metadata = scanner.scan_all_khipu()
