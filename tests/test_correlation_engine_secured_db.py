...
from mallku.core.database.secured_interface import CollectionSecurityPolicy
...
class TestCorrelationEngineSecuredDBIntegration:
    ...
    @pytest.mark.asyncio
    async def test_memory_anchor_creation_through_secured_path(self, correlation_engine, secured_db):
    ...
    async def test_query_service_uses_secured_database(self, secured_db):
        ...
        assert "memory_anchors" in [p.collection_name for p in secured_db._collection_policies.values()]
