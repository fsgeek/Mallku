...
async def test_reciprocity_consciousness_integration():
    ...
    try:
        response = await prompt_manager.execute_prompt(
            category=PromptCategory.DATABASE_VALIDATION, prompt=test_prompt, context=test_context
        )
        ...
        if call_log:
            ...
            prompt_preview = (
                enhanced_call.prompt[:200] + "..."
                if len(enhanced_call.prompt) > 200
                else enhanced_call.prompt
            )
        ...
        print(
            f"Consciousness integration active: {health_metrics['system_self_awareness']['consciousness_integration_active']}"
        )
        print(
            f"Total interactions: {health_metrics['system_self_awareness']['total_llm_interactions']}"
        )
        ...
        print(
            f"Collective consciousness integration: {fire_circle_status['collective_consciousness_integration']}"
        )
...
async def demonstrate_system_consciousness_evolution():
    ...
    responses = [
        ...
        (
            "I want to build this with care for future developers who will inherit our work",
            0.8,
        ),  # Cathedral-focused
        (
            "We can create something that serves the community's flourishing through careful attention to reciprocal design patterns",
            0.9,
    ...
    for i, (response_text, expected_score) in enumerate(responses, 1):
        ...
        print(
            f"Before: Health {prompt_manager.reciprocity_health_score:.2f}, Stage: {prompt_manager.current_transformation_stage.value}"
        )
        ...
        print(
            f"After: Health {prompt_manager.reciprocity_health_score:.2f}, Stage: {prompt_manager.current_transformation_stage.value}"
        )
        print()
