...
class TestCorrelationEngineSecuredDBIntegration:
    ...
    def mock_secured_db(self, mock_arango_collection):
        ...
        db.get_security_metrics = MagicMock(return_value={
            "operations_count": 5,
    ...
    async def memory_service(self, mock_secured_db):
        ...
        with patch('mallku.services.memory_anchor_service.get_secured_database', return_value=mock_secured_db):
    ...
    @pytest.mark.asyncio
    async def test_correlation_engine_uses_secured_database(self, correlation_engine, mock_secured_db):
    ...
    @pytest.mark.asyncio
    async def test_memory_anchor_creation_through_secured_path(self, correlation_engine, mock_secured_db, mock_arango_collection):
    ...
    async def test_query_service_uses_secured_database(self, mock_secured_db):
        ...
        with patch('mallku.query.service.get_secured_database', return_value=mock_secured_db):
            ...
            query_request = QueryRequest(
                query_text="files from last hour",
                max_results=10
            )
    ...
    async def test_secured_db_prevents_direct_access(self):
        ...
        with patch('mallku.core.database.factory.importlib.util.spec_from_file_location') as mock_spec:
            ...
            with patch('mallku.core.database.factory.importlib.util.module_from_spec', return_value=mock_module):
...
class TestCorrelationEngineMemoryAnchorIntegration:
    ...
    async def test_memory_anchor_creation_fields(self, mock_secured_db_with_collection):
        ...
        with patch('mallku.correlation.engine.get_secured_database', return_value=mock_secured_db_with_collection):
            engine = CorrelationEngine()
