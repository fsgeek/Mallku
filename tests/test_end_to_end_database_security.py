...
class TestEndToEndDatabaseSecurity:
    ...
    async def test_complete_security_flow(
        self, secured_database, correlation_engine, query_service
    ):
        """
        ...
        assert not collection._policy.requires_security  # Legacy compatibility
        ...
        matching_results = [r for r in results if test_file_path in str(r.file_path)]
        ...
        assert result.correlation_type in [
            "sequential",
            "temporal_proximity",
        ...
        if anchor_context and "anchor" in anchor_context:
            ...
            if "correlation_id" in anchor["metadata"]:
                assert anchor["metadata"]["pattern_type"] in [
                    "sequential",
                    "concurrent",
                    "cyclical",
                    "contextual",
    ...
    @pytest.mark.asyncio
    async def test_performance_characteristics(self, correlation_engine, query_service):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_operations(self, correlation_engine, query_service):
        ...
        tasks = [create_event_stream(f"stream_{i}", 10) for i in range(3)]
...
class TestMemoryAnchorSecurityPolicy:
    ...
    async def test_memory_anchor_policy_enforcement(self, secured_database):
        ...
        assert not policy.requires_security
    ...
    async def test_secured_collection_access_patterns(self, secured_database):
        ...
        unsafe_methods = ["insert", "update", "delete", "truncate"]
