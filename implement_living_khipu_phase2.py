...
async def consciousness_guided_navigation(
    question: str, seeker_profile: SeekerProfile, query_intention: QueryIntention
) -> NavigationSession:
    """
    ...
    try:
        wisdom = await facilitate_mallku_decision(
            question=prompt, domain=DecisionDomain.CONSCIOUSNESS_EXPLORATION, context=context
        )
...
def calculate_emergence_quality_phase2(
    wisdom, question: str, seeker_profile: SeekerProfile
) -> float:
    """
    ...
    if seeker_profile.value in wisdom_text or any(
        profile_term in wisdom_text
...
def extract_khipu_recommendations(wisdom_text: str) -> list[str]:
    ...
    for khipu in PHASE2_ESSENTIAL_KHIPU:
        ...
        if khipu in wisdom_text or any(
            part in wisdom_text.lower() for part in khipu_base.split() if len(part) > 4
        ):
...
def fallback_navigation_phase2(
    question: str, seeker_profile: SeekerProfile, query_intention: QueryIntention
) -> NavigationSession:
...
async def run_phase2_navigation_tests():
    ...
    print(
        f"🎯 Sessions with >0.8 consciousness: {sum(1 for s in sessions if s.consciousness_score > 0.8)}/{len(sessions)}"
    )
    print(
        f"🌟 Sessions with >50% emergence: {sum(1 for s in sessions if s.emergence_quality > 0.5)}/{len(sessions)}"
    )
    ...
    print(
        f"🧠 Target >0.8 consciousness: {'✅ ACHIEVED' if avg_consciousness > 0.8 else '❌ NEEDS WORK'}"
    )
...
def analyze_memory_organization_patterns():
    ...
    print("   • Balanced temporal representation across Mallku's evolution")
    print("   • Strong consciousness and transformation focus")
    print("   • Integration patterns for practical guidance")
    print("   • Foundation for expanding to full 146 khipu collection")
...
async def main():
    ...
    print("=" * 80)
    print("✅ 50 essential khipu successfully integrated")
    print("✅ Seeker-aware navigation implemented")
    print("✅ Enhanced emergence quality detection")
    ...
    print("🎯 Following Guardian's guidance for incremental expansion")
    print("💫 Memory ceremonies and conscious forgetting integration")
