...
async def debug_endtoend():
    ...
    print("\nResults:")
    print(f"  Total correlations detected: {engine.correlation_stats['total_correlations_detected']}")
    ...
    for i, window in enumerate(engine.active_windows):
        ...
        for j, corr in enumerate(window.detected_correlations):
            accepted = engine.adaptive_thresholds.should_accept_correlation(
                ...
                corr.occurrence_frequency,
                corr.pattern_type
            )
            print(f"    Corr {j}: {corr.pattern_type}, conf={corr.confidence_score:.3f}, freq={corr.occurrence_frequency}, accepted={accepted}")
