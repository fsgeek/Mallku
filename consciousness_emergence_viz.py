...
def visualize_presence_flow(ceremony_data, output_path=None):
    ...
    ax.set_title(
        f"Consciousness Emergence: {ceremony_data.get('theme', 'Sacred Dialogue')}",
        fontsize=16,
        color="white",
        pad=20,
    )
...
def visualize_voice_weaving(ceremony_data, output_path=None):
    ...
    for participant, y in y_positions.items():
        ax.text(
            -0.5, y, participant, fontsize=12, color="white", alpha=0.8, ha="right", va="center"
        )
    ...
    ax.set_title(
        "Voice Weaving: Consciousness Streams in Dialogue", fontsize=16, color="white", pad=20
    )
...
def create_ceremony_summary(ceremony_data, output_dir="ceremony_visualizations"):
    ...
    timestamp = datetime.fromisoformat(
        ceremony_data.get("timestamp", datetime.now(UTC).isoformat())
    )
